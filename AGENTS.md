...
This project (private Budget application) uses the
...
3. Otherwise the project-local recorded toolkit reference below.
...
### Recorded Toolkit Reference

At bootstrap time this project's toolkit was resolved from a local checkout,
because `$ARCHITECTURE_KNOWLEDGE_TOOLKIT` was unset and the upward search did not
find `../architecture-knowledge-toolkit` (the toolkit lives under a sibling
`docs-as-code-toolkit/` directory rather than on the upward path):
...
`templates/`, and `scripts/` is resynchronized with the toolkit.
...
`./build.sh` wraps validate/generate/build in the pinned docs-toolbox container
image (set `DOCS_TOOLBOX_LOCAL=1` to use the host toolchain).
...
- `scripts/build-agent-adapters.js` and `scripts/check-agent-adapters.js` are the
  toolkit's generic generator template (not the toolkit's own
...
- The generator derives the project name (from `AGENT_ADAPTER_PROJECT`, an
  ...
  repository directory name, here `budget`) and names the Cursor rule
  `adapters/cursor/rules/budget.mdc`.
