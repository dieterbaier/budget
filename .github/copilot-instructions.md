...
This project generates agent adapters (see `adapters/` and
`scripts/build-agent-adapters.js`), so keep this file as an entry point only:
...
duplicate architecture or SDLC rules here.
...
Prefer a stable release tag or commit SHA for long-lived project references (see
the recorded toolkit reference in `AGENTS.md`).
