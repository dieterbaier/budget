...
When one of them fails, the detail is in a report rather than the console output:

| Report | Open |
|---|---|
| Backend tests and Gherkin scenarios | `application/backend/build/reports/tests/test/index.html` |
| Architecture rules alone | `application/backend/build/reports/tests/architectureTest/index.html` |
| Backend coverage | `application/backend/build/reports/jacoco/test/html/index.html` |
| Web client coverage | `application/web/coverage/index.html` |

All four are git-ignored build output, regenerated by the command that produced
them.
